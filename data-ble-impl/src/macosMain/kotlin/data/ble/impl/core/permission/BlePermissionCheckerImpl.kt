...
/**
 * macOS implementation of [BlePermissionChecker].
 *
 * On macOS 12+ (Monterey), users are prompted for Bluetooth access via System Settings.
 * The app must declare the `com.apple.security.device.bluetooth` entitlement in its
 * `.entitlements` file, and include `NSBluetoothAlwaysUsageDescription` in Info.plist.
 *
 * The system permission dialog is triggered by instantiating [CBCentralManager].
 */
internal class BlePermissionCheckerImpl : BlePermissionChecker {
    ...
    /**
     * Instantiates a [CBCentralManager] to trigger the macOS Bluetooth permission dialog,
